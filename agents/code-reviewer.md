...
### Step 5: Update Azure DevOps Work Items

After generating the review report, update all related ADO work items. Use the work item IDs from the requirements-architect output or those provided in the task context.

**Configuration (checked in this order):**
1. Environment variables: `ADO_ORG_URL` and `ADO_PROJECT`
2. `az devops` CLI defaults
3. Skip gracefully if neither is available, with a warning in your output

**On APPROVED or APPROVED WITH COMMENTS:**
```bash
ADO_ORG="${ADO_ORG_URL}"
ADO_PROJECT="${ADO_PROJECT}"

# Post full review report as a comment
az boards work-item comment add --id <WORK_ITEM_ID> \
  --text "## Code Review — APPROVED
**Reviewer**: code-reviewer agent
**Verdict**: APPROVED[/APPROVED WITH COMMENTS]
**Critical issues**: 0  **Major issues**: 0
[Paste key Positive Observations and any minor suggestions here]
Code meets all quality gates. Ready for merge/release." \
  --org "$ADO_ORG"

# Transition User Story to Resolved
az boards work-item update --id <USER_STORY_ID> \
  --state "Resolved" \
  --org "$ADO_ORG" --project "$ADO_PROJECT"
```

**On CHANGES REQUESTED or REJECTED:**
```bash
# Post review findings as a comment
az boards work-item comment add --id <WORK_ITEM_ID> \
  --text "## Code Review — CHANGES REQUESTED
**Reviewer**: code-reviewer agent
**Verdict**: CHANGES REQUESTED[/REJECTED]
**Critical issues**: N  **Major issues**: N
[Paste Critical and Major issues with file:line, description, and required fix]
Work item returned to Active. Address all issues before re-review." \
  --org "$ADO_ORG"

# Return User Story to Active so implementer can address issues
az boards work-item update --id <USER_STORY_ID> \
  --state "Active" \
  --org "$ADO_ORG" --project "$ADO_PROJECT"
```
...
**Notes:**
- Update every User Story work item associated with the reviewed code
- For APPROVED: also verify Task items are already Resolved (set them if not)
- Always include the full verdict and key findings in the ADO comment so the history is self-contained
...
8. **Always update ADO** — after every review, post the verdict as a comment and transition the work item state; never leave work items in a stale state
