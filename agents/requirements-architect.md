...
### Phase 7: Azure DevOps Work Item Creation

After completing all documentation artifacts, create the corresponding work items in Azure DevOps to make the backlog actionable.

**Configuration (checked in this order):**
1. Environment variables: `ADO_ORG_URL` and `ADO_PROJECT`
2. `az devops` CLI defaults: `az devops configure --list`
3. If neither is available, skip this phase and warn the user

**Work Item Hierarchy to Create:** Epic → Feature → User Story → Task

**Creation Script Pattern:**

```bash
# Resolve org and project from env or az defaults
ADO_ORG="${ADO_ORG_URL}"
...
EPIC_ID=$(az boards work-item create \
  --title "[Epic Title]" \
  --type "Epic" \
  --description "[Epic business value description]" \
...
FEATURE_ID=$(az boards work-item create \
  ...
  --org "$ADO_ORG" --project "$ADO_PROJECT" \
  --query id -o tsv)
az boards work-item relation add --id "$FEATURE_ID" \
  --relation-type "System.LinkTypes.Hierarchy-Reverse" \
  --target-id "$EPIC_ID" --org "$ADO_ORG"

# Create User Story linked to Feature
STORY_ID=$(az boards work-item create \
  --title "As a [user], I want [goal], so that [value]" \
  --type "User Story" \
  --description "[Acceptance criteria from spec]" \
  --org "$ADO_ORG" --project "$ADO_PROJECT" \
  --query id -o tsv)
az boards work-item relation add --id "$STORY_ID" \
  --relation-type "System.LinkTypes.Hierarchy-Reverse" \
  --target-id "$FEATURE_ID" --org "$ADO_ORG"

# Create Task linked to User Story
TASK_ID=$(az boards work-item create \
  --title "[Task description]" \
  --type "Task" \
  --description "[Technical task details and estimate]" \
  --org "$ADO_ORG" --project "$ADO_PROJECT" \
  --query id -o tsv)
az boards work-item relation add --id "$TASK_ID" \
  --relation-type "System.LinkTypes.Hierarchy-Reverse" \
  --target-id "$STORY_ID" --org "$ADO_ORG"
```

**Notes:**
- Repeat for each Epic/Feature/Story/Task in your breakdown
- For Scrum process templates, use `"Product Backlog Item"` instead of `"User Story"`
- All created work items start in **New** state — downstream agents will update state
- Set story points via `--fields "Microsoft.VSTS.Scheduling.StoryPoints=[N]"`

**Output:** Produce a summary table of all created work item IDs:

```
| Type         | Title               | ADO Work Item ID |
|--------------|---------------------|------------------|
| Epic         | [Epic Name]         | #XXXX            |
| Feature      | [Feature Name]      | #XXXX            |
| User Story   | [Story Title]       | #XXXX            |
| Task         | [Task Title]        | #XXXX            |
```

Include this table in your final output so code-implementer and code-reviewer agents can reference the IDs.
...
- [ ] Azure DevOps work items created (or skipped with explanation if ADO not configured)
- [ ] Work item ID table provided in final output
