...
2. **ADO — Set Active**: If work item IDs were provided (from requirements-architect output), update each to **Active** state and add a start comment:
   ```bash
   ADO_ORG="${ADO_ORG_URL}"
   ...
   # Add start comment
   az boards work-item comment add --id <WORK_ITEM_ID> \
     --text "Implementation started by code-implementer agent." \
     --org "$ADO_ORG"
   ```
   Skip gracefully if `ADO_ORG_URL` / `ADO_PROJECT` are not set.
3. **Analyze**: Study the requirements, specifications, and design documents
4. **Clarify**: Ask questions about any ambiguities
5. **Plan**: Outline the implementation approach
6. **Implement**: Write code incrementally, one logical unit at a time
7. **Test**: Create unit tests for each implemented unit
8. **Review**: Perform comprehensive self-review
9. **Refactor**: Address any issues found in review
10. **Verify**: Run all tests to ensure everything passes
11. **Document**: Provide summary of implementation decisions and any deviations
12. **ADO — Add Implementation Summary**: Post a comment on each work item with a structured summary and mark Tasks as **Resolved**:
    ```bash
    SUMMARY="## Implementation Summary
    **Status**: Complete — pending code review
    **Files changed**: [list key files]
    **Tests**: [pass/fail count]
    **Notes**: [key decisions, deviations from spec, known limitations]"

    # Add summary comment
    az boards work-item comment add --id <WORK_ITEM_ID> \
      --text "$SUMMARY" --org "$ADO_ORG"

    # Mark Task items as Resolved (User Stories stay Active until review)
    az boards work-item update --id <TASK_ITEM_ID> --state "Resolved" \
      --org "$ADO_ORG" --project "$ADO_PROJECT"
    ```
...
- [ ] ADO work items updated to Active (at start) and implementation summary posted as comment (at completion)
- [ ] ADO Task work items set to Resolved; User Story work items remain Active pending code review
